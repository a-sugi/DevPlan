...
BEGIN
	...
	/* 2018.12.19 プリントスクリーン(ROLL_ID=9)を除外 */
	/* 2019.09.27 トラック利用者(ROLL_ID=16)、トラック管理者(ROLL_ID=17)を除外 */
	DELETE FROM DEVPLAN.ROLL_AUTH WHERE NOT(ROLL_ID = '14' AND PERSONEL_ID IS NOT NULL) AND ROLL_ID NOT IN ('9', '16', '17');
	...
	FOR rec IN (
		SELECT DISTINCT
			...
			FROM
				DEPARTMENT_DATA 
			WHERE
				DEPARTMENT_ID NOT IN ('57','69','70','73','74','75','81','88') -- STC以外
	...
	FOR rec IN (
		SELECT
			...
			WHERE
				...
				AND C.DEPARTMENT_ID NOT IN ('57','69','70','73','74','75','81','88') -- STC以外
	...
	FOR rec IN (
		SELECT
			...
			WHERE
				...
				AND A.SECTION_GROUP_ID = B.SECTION_GROUP_ID
				AND B.SECTION_ID = C.SECTION_ID
				AND C.DEPARTMENT_ID IN ('57','69','70','73','74','75','81','88') -- STCのみ
				AND B.FLAG_EXIST = 1  -- 有効のみ
			ORDER BY
				A.PERSONEL_ID
	) LOOP
...
*/
	...
	/* 総括部署ロールへの部署の割り当て（部） */
	vIDCount := vIDCount + 1;
	INSERT INTO DEVPLAN.ROLL_AUTH VALUES (vIDCount,'3',NULL,NULL,NULL,NULL,'12',SYSDATE,'00001',NULL,NULL);
	vIDCount := vIDCount + 1;
	INSERT INTO DEVPLAN.ROLL_AUTH VALUES (vIDCount,'23',NULL,NULL,NULL,NULL,'12',SYSDATE,'00001',NULL,NULL);
...
/*	FOR rec IN (
		...
		vIDCount := vIDCount + 1;
		INSERT INTO DEVPLAN.ROLL_AUTH VALUES (vIDCount,rec.DEPARTMENT_ID,NULL,NULL,NULL,NULL,'16',SYSDATE,'00001',NULL,NULL);

	END LOOP;
*/
	/* トラック管理者ロールへの担当の割り当て（担当） */
/*	FOR rec IN (
		SELECT DISTINCT 
			D.DEPARTMENT_ID,
			C.SECTION_ID,
			...
			FROM
				...
				DEPARTMENT_DATA D
			WHERE
				A.SECTION_GROUP_ID = B.SECTION_GROUP_ID
				AND B.SECTION_ID = C.SECTION_ID
				AND C.DEPARTMENT_ID = D.DEPARTMENT_ID
				AND B.FLAG_EXIST = 1  -- 有効のみ
			ORDER BY
		...
		vIDCount := vIDCount + 1;
		INSERT INTO DEVPLAN.ROLL_AUTH VALUES (vIDCount,rec.DEPARTMENT_ID,rec.SECTION_ID,rec.SECTION_GROUP_ID,NULL,NULL,'17',SYSDATE,'00001',NULL,NULL);

	END LOOP;
*/
	/* CAP担当部署ロールへの担当の割り当て（担当） */
	FOR rec IN (
		SELECT DISTINCT 
			D.DEPARTMENT_ID,
			C.SECTION_ID,
		...
		vIDCount := vIDCount + 1;
		INSERT INTO DEVPLAN.ROLL_AUTH VALUES (vIDCount,rec.DEPARTMENT_ID,rec.SECTION_ID,rec.SECTION_GROUP_ID,NULL,NULL,'18',SYSDATE,'00001',NULL,NULL);

	END LOOP;

	/* CAP自部出力利用者ロールへの人の割り当て（人） */
	FOR rec IN (
		SELECT DISTINCT
			A.PERSONEL_ID
			FROM
		...
		vIDCount := vIDCount + 1;
		INSERT INTO DEVPLAN.ROLL_AUTH VALUES (vIDCount,NULL,NULL,NULL,NULL,rec.PERSONEL_ID,'19',SYSDATE,'00001',NULL,NULL);

	END LOOP;

	/* SKSロールへの部署の割り当て（部） */
	vIDCount := vIDCount + 1;
	INSERT INTO DEVPLAN.ROLL_AUTH VALUES (vIDCount,'17',NULL,NULL,NULL,NULL,'20',SYSDATE,'00001',NULL,NULL);

	/* 設計チェック出力利用者ロールへの人の割り当て（人） */
	FOR rec IN (
		SELECT DISTINCT
			A.PERSONEL_ID
			FROM
		...
		vIDCount := vIDCount + 1;
		INSERT INTO DEVPLAN.ROLL_AUTH VALUES (vIDCount,NULL,NULL,NULL,NULL,rec.PERSONEL_ID,'21',SYSDATE,'00001',NULL,NULL);
