...
namespace DevPlanWebAPI.Logic
{
    ...
    /// 業務ロジッククラス
    /// </summary>
    /// <remarks>カーシェア内製車検索</remarks>
    public class CarShareInnerLogic : BaseLogic
    {
        /// <summary>
        /// カーシェア内製車データの取得
        /// </summary>
        /// <returns>DataTable</returns>
        public IEnumerable<CarShareInnerSearchOutModel> GetData(CarShareInnerSearchInModel val)
        {
            var prms = new List<BindModel>();
            var sql = new StringBuilder();

            sql.AppendLine("SELECT");
            sql.AppendLine("     K.CAR_GROUP 車系");
            ...
            sql.AppendLine("    LEFT JOIN GENERAL_CODE K ON G.開発符号 = K.GENERAL_CODE");
            ...
            sql.AppendLine("                    ) L");
            sql.AppendLine("     ON L.管理票番号 = H.管理票NO");
            ...
            sql.AppendLine("    AND G.外製車名 IS NULL");
            sql.AppendLine("    AND G.登録ナンバー IS NOT NULL");
            //sql.AppendLine("    AND C.SECTION_ID = '367'");
            //スケジュールに登録されてる車両は除外
            ...
            */
            ...
            //車系
            if (!string.IsNullOrWhiteSpace(val.車系))
            {
                sql.AppendLine("    AND K.CAR_GROUP = :CAR_GROUP");

                prms.Add(new BindModel { Name = ":CAR_GROUP", Type = OracleDbType.Varchar2, Object = val.車系, Direct = ParameterDirection.Input });
            }
            ...
            return db.ReadModelList<CarShareInnerSearchOutModel>(sql.ToString(), prms);
        }
    }
}