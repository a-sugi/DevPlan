...
namespace DevPlanWebAPI.Logic
{
    ...
    /// お気に入り（外製車）業務ロジッククラス
    /// </summary>
    public class OuterCarFavoriteLogic : BaseLogic
    {
        #region お気に入り（外製車）検索結果取得
        /// <summary>
        /// お気に入り（外製車）検索結果取得
        /// </summary>
        /// <param name="search">検索条件</param>
        /// <returns></returns>
        public IEnumerable<OuterCarFavoriteSearchOutModel> Get(OuterCarFavoriteSearchInModel search)
        {
            ...
            sql.AppendLine("    ,MAKER");
            sql.AppendLine("    ,CAR_NAME");
            ...
            sql.AppendLine("FROM");
            sql.AppendLine("    FAVORITE_OUTSOURCE_CAR");
            ...
            //取得
            return db.ReadModelList<OuterCarFavoriteSearchOutModel>(sql.ToString(), prms);

        }
        #endregion

        #region お気に入り（外製車）項目追加
        /// <summary>
        /// お気に入り（外製車）項目追加
        /// </summary>
        /// <param name="favorite">お気に入り（外製車）項目</param>
        /// <returns>追加可否</returns>
        public bool Insert(OuterCarFavoriteItemModel favorite)
        {
            //トランザクション開始
            db.Begin();
            ...
            //SQL
            var sql = new StringBuilder();
            sql.AppendLine("INSERT INTO FAVORITE_OUTSOURCE_CAR");
            sql.AppendLine("(");
            sql.AppendLine("    ID");
            sql.AppendLine("    ,TITLE");
            sql.AppendLine("    ,PERSONEL_ID");
            ...
            sql.AppendLine("(");
            sql.AppendLine("     DEVPLAN.SEQ_FAVORITE_OUTSOURCE_CAR_1.NEXTVAL");
            sql.AppendLine("    ,:TITLE");
            sql.AppendLine("    ,:PERSONEL_ID");
            sql.AppendLine("    ,:CAR_GROUP");
            sql.AppendLine("    ,:MAKER");
            ...
            var prms = new List<BindModel>
            {
                ...
                new BindModel { Name = ":MAKER", Type = OracleDbType.Varchar2, Object = favorite.MAKER, Direct = ParameterDirection.Input },
                new BindModel { Name = ":CAR_NAME", Type = OracleDbType.Varchar2, Object = favorite.CAR_NAME, Direct = ParameterDirection.Input },
