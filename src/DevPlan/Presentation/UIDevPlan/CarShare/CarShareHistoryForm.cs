...
namespace DevPlan.Presentation.UIDevPlan.CarShare
{
    ...
    /// 作業履歴(カーシェア)
    /// </summary>
    public partial class CarShareHistoryForm : BaseSubDialogForm
    {
        #region メンバ変数
        ...
        #region プロパティ
        /// <summary>画面名</summary>
        public override string FormTitle { get { return "作業履歴(カーシェア)"; } }
        ...
        public CarShareScheduleItemModel ScheduleItem { get; set; }
        ...
        /// <param name="e"></param>
        private void CarShareHistoryForm_Load(object sender, EventArgs e)
        {
            //画面初期化
            this.InitForm();
        ...
        private void InitForm()
        {
            ...
            var isCarChare = base.GetFunction(FunctionID.CarShare).CARSHARE_OFFICE_FLG == '1';
            ...
            //ステータス
            this.StatusPanel.Enabled = isManagement || (isJibuManagement && !string.IsNullOrWhiteSpace(this.ScheduleItem.INPUT_SECTION_ID));

            //車両名の編集ボタン
            this.ScheduleItemEditButton.Visible = isManagement || (isJibuManagement && !string.IsNullOrWhiteSpace(this.ScheduleItem.INPUT_SECTION_ID));
            ...
            //フォーム位置変更(親画面の中央に配置する)
            this.Location = new Point(this.Owner.Location.X + (this.Owner.Width - this.Width) / 2,
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void CarShareHistoryForm_Shown(object sender, EventArgs e)
        {
            //一覧を未選択状態に設定
        ...
        private void FavoriteComboBox_SelectedValueChanged(object sender, EventArgs e)
        {
            ...
            //スケジュール項目と作業履歴を設定及びスクロールリセット
            this.HistoryMultiRow.FirstDisplayedLocation = Point.Empty;
            this.SetItemHistoryByFavoriteID(item.ID);

            //ボタン設定
            this.StatusPanel.Enabled = this.ScheduleItemEditButton.Visible 
                = isManagement || (isJibuManagement && !string.IsNullOrWhiteSpace(this.ScheduleItem.INPUT_SECTION_ID));
        }
        #endregion
        ...
        private void FavoriteEditButton_Click(object sender, EventArgs e)
        {
            ...
            {
                // データ区分
                CLASS_DATA = Const.FavoriteCarShareWorkHistory
        ...
        {
            var item = new ScheduleItemModel<CarShareScheduleItemModel>()
            {
                // ID
                ID = ScheduleItem.ID,
        ...
        private void RowDeleteButton_Click(object sender, EventArgs e)
        {
            ...
            var isManagement = this.UserAuthority.MANAGEMENT_FLG == '1';
            var isJibuManagement = this.UserAuthority.JIBU_MANAGEMENT_FLG == '1';
            var isCrashareOffice = this.UserAuthority.CARSHARE_OFFICE_FLG == '1';

            // 編集許可チェック（他部署）
            if (!string.IsNullOrWhiteSpace(workHistory.INPUT_SECTION_CODE) &&
                workHistory.INPUT_SECTION_CODE != SessionDto.SectionCode)
            {
                var msg = GetHistoryEditMessage(isManagement, isJibuManagement, isCrashareOffice);
        ...
        /// 他部署変更チェック。
        /// </summary>
        /// <param name="isManagement">管理フラグ</param>
        /// <param name="isJibuManagement">自部署フラグ</param>
        /// <param name="isCrashareOffice">カーシェア事務所フラグ</param>
        /// <returns></returns>
        private string GetHistoryEditMessage(bool isManagement, bool isJibuManagement, bool isCrashareOffice)
        {
            string msg = string.Empty;

            if (string.IsNullOrWhiteSpace(this.ScheduleItem.INPUT_SECTION_ID))//管理
            {
                if (!isManagement)
                {
                    msg = "SJSB";
            ...
            {
                if (!isManagement && !isJibuManagement && !isCrashareOffice)//自部署
                {
                    msg = "SJSB,SKCKA";
                }
                else
            ...
            if (!string.IsNullOrWhiteSpace(msg)) { msg = string.Format(Resources.KKM03036, msg); }
        ...
        private void FavoriteEntryButton_Click(object sender, EventArgs e)
        {
            ...
            {
                //履歴区分
                HISTORY_CODE = Const.HistoryCarShare,
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void CarShareHistoryForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            FormControlUtil.FormWait(this, () =>
            {
                //作業履歴の編集は終了
        ...
        /// <param name="id">ID</param>
        /// <returns></returns>
        private CarShareScheduleItemModel GetScheduleItem(long id)
        {
            //パラメータ設定
            var cond = new CarShareScheduleSearchModel { ID = id };
        ...
        private List<ScheduleToXeyeOutModel> GetXeyeUrl()
        {
            ...
            searchModel.物品名2 = this.ScheduleItem.管理票番号;
        ...
        private bool EntryWorkHistory()
        {
            ...
            {
                //作業履歴登録
                var res = HttpUtil.PutResponse(ControllerType.CarShareWorkHistory, list);
