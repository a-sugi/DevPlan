...
namespace DevPlan.Presentation.UIDevPlan.DesignCheck
{
    ...
    /// 試験車一覧(設計チェック)
    /// </summary>
    public partial class DesignCheckTestCarListForm : BaseSubDialogForm
    {
        #region メンバ変数
        /// <summary>
        /// カスタムテンプレート
        ...
        #region プロパティ
        /// <summary>画面名</summary>
        public override string FormTitle { get { return "試験車一覧(設計チェック)"; } }
        ...
        /// コンストラクタ
        /// </summary>
        public DesignCheckTestCarListForm()
        {
            InitializeComponent();
            ...
            FormControlUtil.FormWait(this, () =>
            {
                foreach (var row in this.ListMultiRow.Rows)
                {
                    ...
                    if (Convert.ToBoolean(row.Cells["SelectedColumn"].Value) == true)
                    {
                        ...
                        list.Add(this.TestCarList.First(x => x.ID == id));
                    }
                }
        ...
        {
            //APIで取得
            var res = HttpUtil.GetResponse<DesignCheckTestCarGetInModel, DesignCheckTestCarGetOutModel>(ControllerType.DesignCheckTestCar, new DesignCheckTestCarGetInModel());
